...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    ...
    twist_msg.linear.x = ps5_joy.getAxisValue(joy::PS5ButtonMap::L3Vertical);  // 左摇杆前后控制x速度
    twist_msg.angular.z = ps5_joy.getAxisValue(joy::PS5ButtonMap::L3Horizontal);  // 左摇杆左右控制yaw速度
    twist_msg.linear.z = ps5_joy.getAxisValue(joy::PS5ButtonMap::R3Vertical);  // 右摇杆前后控制z速度
